...
function handle_signed_message($address, $signedMessage, $nonceFromClient) {
    ...
    if ($signatureValid) {
        ...
        if (is_user_logged_in()) {
            ...
            update_user_meta($current_user->ID, 'looppress_api_key', $proxyApiKey);
            ...
            if (!$user_id) {
                ...
                if (!is_wp_error($user_id)) {
                    // Set the role to looppress_user
            ...
            $proxyApiKey = hash('sha256', $address . time());
            update_user_meta($user_id, 'looppress_api_key', $proxyApiKey);
...
function verify_signature_remotely($nonce, $signedMessage, $address) {
    ...
    $url = 'https://looppress.dev/wp-content/plugins/LoopPress/core/license/signing-endpoint.php';

    // Retrieve the LoopPress license key from WordPress options
    $looppress_license = get_option('looppress_license');
    ...
    $body = array(
        ...
        'address' => $address,
        'looppress_license' => $looppress_license
    );
