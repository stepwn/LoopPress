...
add_action('woocommerce_coupon_data_tabs', 'add_nft_coupon_data_tab', 10, 1);
function add_nft_coupon_data_tab($tabs) {
    $tabs['looppress'] = array(
        'label'   => __('LoopPress', 'your-text-domain'),
...
add_action('woocommerce_coupon_data_panels', 'add_nft_coupon_data_panel');
function add_nft_coupon_data_panel() {
    ?>
...
add_action('woocommerce_coupon_options_save', 'save_nft_coupon_data');
function save_nft_coupon_data($post_id) {
    // NFT Field
...
add_action('woocommerce_product_data_panels', 'add_custom_fields_to_product_tab');
function add_custom_fields_to_product_tab() {
    ?>
...
add_filter('woocommerce_add_to_cart_validation', 'validate_nft_before_add_to_cart', 10, 3);
function validate_nft_before_add_to_cart($passed, $product_id, $quantity) {
    ...
    foreach ($user_nfts as $user_nft) {
        if (check_conditions($user_nft, array('minter' => $required_minter, 'nft' => $required_nft, 'contract' => $required_contract, 'schedule' => $required_schedule))) {
            $has_access = true;
            break;
        }
    }
...
add_action('woocommerce_check_cart_items', 'validate_nft_before_checkout');
function validate_nft_before_checkout() {
...
add_action('woocommerce_check_cart_items', 'validate_nft_coupon_at_checkout');

function validate_nft_coupon_at_checkout() {
    ...
    foreach ( $coupons as $code ) {
        ...
        foreach ( $user_nfts as $user_nft ) {
            if ( check_conditions( $user_nft, array( 'minter' => $minter, 'nft' => $nft, 'contract' => $contract, 'schedule' => $required_schedule) ) ) {
                $has_access = true;
                break;
            }
        }
