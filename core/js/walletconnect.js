...
window.addEventListener('load', async () => {
    ...
    window.watchAccount(account => {
        window.looppress_eth_address = account.address;
        Array.from(document.getElementsByClassName("looppressButton")).forEach(element => {
...
async function looppress_unlock(el) {
    if (window.looppress_eth_address) {
        if(!looppressKey && !window.nonce){
            await sendLoginRequest(window.looppress_eth_address);
        }
    }
    else{
...
async function sendLoginRequest(userAddress) {
    ...
    modal.id = 'looppress_sign_modal';
    modal.style.display = 'none'; // Initially hidden
    modal.innerHTML = `
      <div class="looppress-sign-modal-content">
        <h2>Sign request sent.</h2>
        <p>Check your wallet to sign your login nonce.</p>
        <p id="looppress_sign_nonce"></p>
        <button onclick="document.getElementById('looppress_sign_modal').style.display='none'">Close</button>
      </div>
    `;
  ...
  // Request nonce from the server
    fetch(plugin_dir+'/login-user.php?address='+encodeURIComponent(userAddress))
        ...
        .then(data => {
            ...
            window.looppress_nonce = nonce;
            // display modal with prompt to sign
            document.getElementById('looppress_sign_modal').style.display = "block";
            document.getElementById('looppress_sign_nonce').innerText = nonce;
            ...
            window.signUserMessage(nonce).then(signedNonce => {
                ...
                xhr.onload = function() {
                    if (this.status === 200) {
                        ...
                        document.getElementById('looppress_sign_nonce').innerText = "Login Successful!";
                        //document.getElementById('looppress_sign_modal').style.display = "none";
                        window.location.href = window.location.href;
                    } else {
                        // Handle error
                        console.error('Login failed:', this.status, this.responseText);
                        document.getElementById('looppress_sign_nonce').innerText = "Login Failed.";
                    }
                };
