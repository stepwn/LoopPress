"use strict";
(self["webpackChunkhtml_auth"] = self["webpackChunkhtml_auth"] || []).push([[18],{
...
var utils = __webpack_require__(3486);
...
class SHA2 extends utils/* Hash */.kb {
    ...
    update(data) {
        _assert/* default.exists */.ZP.exists(this);
    ...
    digestInto(out) {
        _assert/* default.exists */.ZP.exists(this);
        _assert/* default.output */.ZP.output(out, this);
        this.finished = true;
        // Padding
 ...
 */
const sha256 = (0,utils/* wrapConstructor */.hE)(() => new SHA256());
const sha224 = (0,utils/* wrapConstructor */.hE)(() => new SHA224());
//# sourceMappingURL=sha256.js.map
// EXTERNAL MODULE: ./node_modules/@noble/curves/esm/abstract/utils.js
var abstract_utils = __webpack_require__(7499);
...
function validateField(field) {
    ...
    return (0,abstract_utils/* validateObject */.FF)(field, opts);
}
...
function Field(ORDER, bitLen, isLE = false, redef = {}) {
    ...
    const f = Object.freeze({
        ...
        BITS,
        BYTES,
        MASK: (0,abstract_utils/* bitMask */.dQ)(BITS),
        ZERO: _0n,
        ONE: _1n,
        ...
        cmov: (a, b, c) => (c ? b : a),
        toBytes: (num) => (isLE ? (0,abstract_utils/* numberToBytesLE */.S5)(num, BYTES) : (0,abstract_utils/* numberToBytesBE */.tL)(num, BYTES)),
        fromBytes: (bytes) => {
            if (bytes.length !== BYTES)
                throw new Error(`Fp.fromBytes: expected ${BYTES}, got ${bytes.length}`);
            return isLE ? (0,abstract_utils/* bytesToNumberLE */.ty)(bytes) : (0,abstract_utils.bytesToNumberBE)(bytes);
        },
    });
...
function hashToPrivateScalar(hash, groupOrder, isLE = false) {
    hash = (0,abstract_utils/* ensureBytes */.ql)('privateHash', hash);
    const hashLen = hash.length;
    const minLen = nLength(groupOrder).nByteLength + 8;
    if (minLen < 24 || hashLen < minLen || hashLen > 1024)
        throw new Error(`hashToPrivateScalar: expected ${minLen}-1024 bytes of input, got ${hashLen}`);
    const num = isLE ? (0,abstract_utils/* bytesToNumberLE */.ty)(hash) : (0,abstract_utils.bytesToNumberBE)(hash);
...
function validateBasic(curve) {
    ...
    (0,abstract_utils/* validateObject */.FF)(curve, {
        n: 'bigint',
        h: 'bigint',
        Gx: 'field',
        Gy: 'field',
...
function weierstrass(curveDef) {
    ...
    function prepSig(msgHash, privateKey, opts = defaultSigOpts) {
        ...
        if (ent != null) {
            ...
            seedArgs.push((0,abstract_utils/* ensureBytes */.ql)('extraEntropy', e, Fp.BYTES)); // check for being of size BYTES
        }
    ...
    function sign(msgHash, privKey, opts = defaultSigOpts) {
        ...
        const drbg = abstract_utils/* createHmacDrbg */.n$(CURVE.hash.outputLen, CURVE.nByteLength, CURVE.hmac);
...
function validateDST(dst) {
    ...
    if (typeof dst === 'string')
        return (0,abstract_utils/* utf8ToBytes */.iY)(dst);
    throw new Error('DST must be Uint8Array or string');
}
// Octet Stream to Integer. "spec" implementation of os2ip is 2.5x slower vs bytesToNumberBE.
const os2ip = abstract_utils.bytesToNumberBE;
...
function expand_message_xmd(msg, DST, lenInBytes, H) {
    ...
    if (DST.length > 255)
        DST = H((0,abstract_utils/* concatBytes */.eV)((0,abstract_utils/* utf8ToBytes */.iY)('H2C-OVERSIZE-DST-'), DST));
    const { outputLen: b_in_bytes, blockLen: r_in_bytes } = H;
    const ell = Math.ceil(lenInBytes / b_in_bytes);
    if (ell > 255)
        throw new Error('Invalid xmd length');
    const DST_prime = (0,abstract_utils/* concatBytes */.eV)(DST, i2osp(DST.length, 1));
    const Z_pad = i2osp(0, r_in_bytes);
    const l_i_b_str = i2osp(lenInBytes, 2); // len_in_bytes_str
    const b = new Array(ell);
    const b_0 = H((0,abstract_utils/* concatBytes */.eV)(Z_pad, msg, l_i_b_str, i2osp(0, 1), DST_prime));
    b[0] = H((0,abstract_utils/* concatBytes */.eV)(b_0, i2osp(1, 1), DST_prime));
    for (let i = 1; i <= ell; i++) {
        const args = [strxor(b_0, b[i - 1]), i2osp(i + 1, 1), DST_prime];
        b[i] = H((0,abstract_utils/* concatBytes */.eV)(...args));
    }
    const pseudo_random_bytes = (0,abstract_utils/* concatBytes */.eV)(...b);
...
function expand_message_xof(msg, DST, lenInBytes, k, H) {
    ...
    if (DST.length > 255) {
        ...
        DST = H.create({ dkLen }).update((0,abstract_utils/* utf8ToBytes */.iY)('H2C-OVERSIZE-DST-')).update(DST).digest();
    }
...
function hash_to_field(msg, count, options) {
    (0,abstract_utils/* validateObject */.FF)(options, {
        DST: 'string',
        p: 'bigint',
        m: 'isSafeInteger',
        k: 'isSafeInteger',
...
class HMAC extends utils/* Hash */.kb {
    constructor(hash, _key) {
        ...
        this.finished = false;
        this.destroyed = false;
        _assert/* default.hash */.ZP.hash(hash);
    ...
    digestInto(out) {
        _assert/* default.exists */.ZP.exists(this);
        _assert/* default.bytes */.ZP.bytes(out, this.outputLen);
        this.finished = true;
...
function taggedHash(tag, ...messages) {
    ...
    if (tagP === undefined) {
        ...
        tagP = (0,abstract_utils/* concatBytes */.eV)(tagH, tagH);
        TAGGED_HASH_PREFIXES[tag] = tagP;
    }
    return sha256((0,abstract_utils/* concatBytes */.eV)(tagP, ...messages));
}
// ECDSA compact points are 33-byte. Schnorr is 32: we strip first byte 0x02 or 0x03
const pointToBytes = (point) => point.toRawBytes(true).slice(1);
const numTo32b = (n) => (0,abstract_utils/* numberToBytesBE */.tL)(n, 32);
...
function challenge(...args) {
    return modN((0,abstract_utils.bytesToNumberBE)(taggedHash('BIP0340/challenge', ...args)));
}
/**
 ...
 */
function schnorrSign(message, privateKey, auxRand = (0,utils/* randomBytes */.O6)(32)) {
    const m = (0,abstract_utils/* ensureBytes */.ql)('message', message);
    const { bytes: px, scalar: d } = schnorrGetExtPubKey(privateKey); // checks for isWithinCurveOrder
    const a = (0,abstract_utils/* ensureBytes */.ql)('auxRand', auxRand, 32); // Auxiliary random data a: a 32-byte array
    const t = numTo32b(d ^ (0,abstract_utils.bytesToNumberBE)(taggedHash('BIP0340/aux', a))); // Let t be the byte-wise xor of bytes(d) and hash/aux(a)
    const rand = taggedHash('BIP0340/nonce', t, px, m); // Let rand = hash/nonce(t || bytes(P) || m)
    const k_ = modN((0,abstract_utils.bytesToNumberBE)(rand)); // Let k' = int(rand) mod n
    if (k_ === secp256k1_0n)
...
function schnorrVerify(signature, message, publicKey) {
    const sig = (0,abstract_utils/* ensureBytes */.ql)('signature', signature, 64);
    const m = (0,abstract_utils/* ensureBytes */.ql)('message', message);
    const pub = (0,abstract_utils/* ensureBytes */.ql)('publicKey', publicKey, 32);
    try {
        const P = lift_x((0,abstract_utils.bytesToNumberBE)(pub)); // P = lift_x(int(pk)); fail if that fails
        const r = (0,abstract_utils.bytesToNumberBE)(sig.subarray(0, 32)); // Let r = int(sig[0:32]); fail if r ≥ p.
        if (!fe(r))
            return false;
        const s = (0,abstract_utils.bytesToNumberBE)(sig.subarray(32, 64)); // Let s = int(sig[32:64]); fail if s ≥ n.
        if (!ge(s))
            return false;
...
const schnorr = {
    ...
    utils: {
        ...
        lift_x,
        pointToBytes,
        numberToBytesBE: abstract_utils/* numberToBytesBE */.tL,
        bytesToNumberBE: abstract_utils.bytesToNumberBE,
        taggedHash,
        mod: mod,
    },
};
